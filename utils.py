...
import numpy as np
from MTANDeepLab import MTANDeepLabv3
...
"""
=========== Universal Multi-task Trainer by Bi-level =========== 
"""
...
def build_meta_model(model, device):
    meta_model = MTANDeepLabv3().to(device)
    meta_model.load_state_dict(model.state_dict())

    model_np = {}
    for n, p in meta_model.named_parameters():
        model_np[n] = p
    return meta_model, model_np

def multi_task_trainer_bilevel(train_loader, 
                               val_loader,
                               test_loader, 
                               multi_task_model, 
                               h,
    ...
    for index in range(total_epoch):
        ...
        for k in tqdm(range(train_batch), total=train_batch):
            # print('batch index', k)
            train_data, train_label, train_depth, train_normal = train_dataset.next()
            train_data, train_label = train_data.to(device, non_blocking=True), train_label.long().to(device, non_blocking=True)
            train_depth, train_normal = train_depth.to(device, non_blocking=True), train_normal.to(device, non_blocking=True)
            
            meta_model, model_np = build_meta_model(multi_task_model, device)
            
            for _ in range(opt.update_num):
                ...
                grads = torch.autograd.grad(loss, model_np.values(), create_graph=True)
            ...
            try:
                ...
                val_data, val_label, val_depth, val_normal = val_dataset.next()
            val_data, val_label = val_data.to(device, non_blocking=True), val_label.long().to(device, non_blocking=True)
            val_depth, val_normal = val_depth.to(device, non_blocking=True), val_normal.to(device, non_blocking=True)
            ...
            del meta_model, val_data, val_label, val_depth, val_normal
            ...
            #               model_fit(train_pred[1], train_depth, 'depth'),
            #               model_fit(train_pred[2], train_normal, 'normal')]
            train_loss = torch.zeros(3).cuda()
            train_loss[0] = model_fit(train_pred[0], train_label, 'semantic')
            train_loss[1] = model_fit(train_pred[1], train_depth, 'depth')
            train_loss[2] = model_fit(train_pred[2], train_normal, 'normal')
            loss = h(train_loss)
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                ...
                test_data, test_label = test_data.to(device, non_blocking=True), test_label.long().to(device, non_blocking=True)
                test_depth, test_normal = test_depth.to(device, non_blocking=True), test_normal.to(device, non_blocking=True)
